...
};const wt = window.loadCardHelpers ? window.loadCardHelpers() : void 0;console.info("%c DECLUTTERING-CARD \n%c   Version 0.4.3   ", "color: orange; font-weight: bold; background: black", "color: white; font-weight: bold; background: dimgray");let bt = class extends K {
  set hass(t) {
    ...
    if (!t.template) throw new Error("Missing template object in your config");const e = function () {
      ...
      if (!t && !e.default) return e.card;let n = [];t && (n = t.slice(0)), e.default && (n = n.concat(e.default));let r = JSON.stringify(e.card);return n.forEach(t => {
        const e = Object.keys(t)[0],
              n = Object.values(t)[0];if ("number" == typeof n || "boolean" == typeof n) {
    ...
    let e;return e = wt ? "divider" === t.type ? (await wt).createRowElement(t) : (await wt).createCardElement(t) : St(t), this._hass && (e.hass = this._hass), e.addEventListener("ll-rebuild", n => {
